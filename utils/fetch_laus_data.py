#!/usr/bin/env python3
"""
Fetch LAUS data defined in data/laus_state_codes.json using BLS API.
...
def load_codes(path=CODES_PATH):
    """
    Load the JSON codes file and flatten to list of (measure, state, series_id).
    """
    ...
    for measure, state_map in data.items():
        for state, sid in state_map.items():
            codes.append((measure, state, sid))
    return codes


def fetch_laus_data(states, start_year, end_year):
    """
    Fetch LAUS series for given states and year range, using codes from the JSON file.
    Saves each series to RAW_DIR/{series_id}.txt with header and monthly data.
    """
    ...
    for b_idx, batch in enumerate(batches, start=1):
        ...
        for year_start in range(start_year, end_year+1, YEAR_SLICE):
            ...
            for series in result["Results"]["series"]:
                ...
                data_pts = [d for d in series.get("data", []) if d.get("period","").startswith("M")]
                if not data_pts:
                ...
                if len(data_pts) != expected:
                    logger.warning(f"{sid} has {len(data_pts)}/{expected} pts in {year_start}-{year_end}")
